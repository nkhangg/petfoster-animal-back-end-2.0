...
public class ReviewServiceImpl implements ReviewService {
    ...
    public ApiResponse createReview(String jwt, ReviewRequest reviewRequest) {
        ...
        if (listOrderDetail.stream().filter(item-> item.getProductRepo().getProduct() == product).toList().size()==0) {
        ...
        Review review = reviewRepository
                .findbyOrderIdAndProductId(reviewRequest.getOrderId(), reviewRequest.getProductId()).orElse(null);
        if (review != null) {
