...
public class AdoptServiceImpl implements AdoptService {
    ...
    public ApiResponse adopt(String jwt, AdoptsRequest adoptsRequest) {
        ...
        if(pet.getAdoptStatus().equalsIgnoreCase(PetStatus.SICK.getValue())) {
            return ApiResponse.builder().status(HttpStatus.BAD_REQUEST.value()).message("This pet is currently sick, so we are unable to proceed with adoption, please comeback when the pet is healthy").errors(true).build();
        }

        if(pet.getAdoptStatus().equalsIgnoreCase(PetStatus.DECEASED.getValue())) {
            return ApiResponse.builder().status(HttpStatus.BAD_REQUEST.value()).message("We unfortunately have to inform you that this pet has already passed away, so adoption is not possible").errors(true).build();
        }
    ...
    public ApiResponse doneAdoption(Integer id) {
        ...
        //was adopted
        if(adopt.getStatus().equalsIgnoreCase(AdoptStatus.ADOPTED.getValue())) {
            return ApiResponse.builder().status(HttpStatus.BAD_REQUEST.value()).message("The adoption was done").errors(true).build();
        }
        
        //hasn't register yet
        if(adopt.getStatus().equalsIgnoreCase(AdoptStatus.WAITING.getValue())) {
            return ApiResponse.builder().status(HttpStatus.BAD_REQUEST.value()).message("The adoption hasn't accepted yet").errors(true).build();
        }
        ...
        if(adopt.getStatus().equalsIgnoreCase(AdoptStatus.CANCELLED_BY_ADMIN.getValue())
            ...
            return ApiResponse.builder().status(HttpStatus.BAD_REQUEST.value()).message("The adoption was cancelled").errors(true).build();
        }
